...
| `digitalocean`| 2026-07-21 | **Unverified.** From DigitalOcean's help docs + a live SRV Create-a-record screenshot; trailing-dot rule confirmed live on emilio.me. |
| `porkbun`     | 2026-07-23 | **Unverified.** From Porkbun's KB add-record (231) + SRV (109) articles; no-trailing-dot confirmed live on wrportal.net. |
...
`clayzahrobsky.com` is already correctly configured on IONOS, so it is a ready-made
target: `verify_thundermail_dns.py clayzahrobsky.com` should report 13/13.
...
`vntc.it` is already correctly configured on OVH (NS `ns200.anycast.me`), so it is a
ready-made target: `verify_thundermail_dns.py vntc.it` should report 13/13, and
...
`emilio.me` is DigitalOcean-hosted (NS `ns1`/`ns2`/`ns3.digitalocean.com`), so it is a
...
so it came back as `mail.thundermail.com.emilio.me` — which both confirms the trailing-dot
...
2. Confirm three things:
   ...
   - Whether targets tolerate no trailing dot (we ship targets verbatim, no dot) — confirmed
     live for MX/CNAME on `wrportal.net`; re-confirm for SRV once SRV records exist there.
3. Fix `records.json` if anything differs, then remove the `UNVERIFIED —` prefixes
   from the four `porkbun` headers, and delete the unverified notes in
   [`README.md`](README.md).

`wrportal.net` is Porkbun-hosted (NS `curitiba`/`fortaleza`/`maceio`/`salvador.ns.porkbun.com`),
...
`soylent.red`). That would confirm the record **values** and the combined-`name`
...
Added 2026-07-16 for internal ticket 6911 (`6911_IONOS_DNS_CLAYZAHROBSKY_DOT_COM`,
domain `clayzahrobsky.com`). Marked **unverified**: the conventions come from six IONOS
...
target needs a trailing dot. To promote: `clayzahrobsky.com` is already correctly
configured on IONOS, so confirm 13/13 with `verify_thundermail_dns.py clayzahrobsky.com`,
...
Added 2026-07-16 for internal ticket 6837 (`6837_OVH_DNS_PROVIDER_vntc.t`, sample domain
`vntc.it`). Marked **unverified**: the conventions come from three OVHcloud help articles
...
trailing dot. To promote: `vntc.it` is already correctly configured on OVH, so confirm 13/13
with `verify_thundermail_dns.py vntc.it` (or `--resolver ns200.anycast.me`), eyeball the
